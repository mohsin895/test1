...
class BookAppoinmentController extends Controller
{
    public function index()
    {
        ...
        $data = Appointment::with(['slot'])->where(['doctor_id' => $userId->doctor_id])->orderBy('id', 'desc')->get();
        $doctor = User::with(['doctor_details','doctor_chambers' => function($query) {
                                return $query->with(['slots', 'chamber']);
                            }])->find($userId->doctor_id);
        $tomorrow = now()->addDay()->format('m-d-Y');
        $today_name = strtolower(now()->format('l'));
        $patientTypes = PatientType::where('status', 1)->get();
        
        return Inertia::render('Compounder/NewAppointment/Index', compact('data', 'doctor', 'tomorrow', 'today_name', 'patientTypes'));
    }
    ...
    { $userId = User::find(auth()->id());
        ...
        }, 'degrees', 'specializations'])->findOrFail($userId->doctor_id);
    ...
    public function save(Request $request)
    {
        ...
        $userId = User::find(auth()->id());
        $user = User::findOrFail($userId->doctor_id);
    ...
    {
        $userId = User::find(auth()->id());
        $data = Appointment::where('doctor_id',$userId->doctor_id)->orderBy('id', 'desc')->get();

        
        $compounder = User::with('compounderDetails','compounderDetails.chamber')->where('id',auth()->id())->first();
        return Inertia::render('Compounder/NewAppointment/AppointmentList', compact('data','compounder'));
    }
    ...
    public function getAppointments(Request $request) {
        ...
        if ($request->slot_id) {
            $slot=CompounderDetails::where('slot_id',$request->slot_id);
            if($slot){
