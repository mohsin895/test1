<template>
  <HospitalLayout :title="`Appointments List (Dr.${doctor.name})`">
...
const doctorId = ref(props.doctor.id);
...
const chamber_id = ref(props.chamber.id || "");
const slot_id = ref("");
const slots = ref([]);
...
const fetchAppointments = async () => {
  try {
    const response = await axios.post(route("hospitals.appointments.getAppointments"), {
      type: type.value,
...
const getSlot = async () => {
  if (chamber_id.value) {
    try {
      const response = await axios.post(route("hospitals.appointments.slot"), {
        chamber_id: chamber_id.value,
      });
...
const handleDelete = async (id) => {
  Helper.confirm("Are you sure to delete?", async () => {
    try {
      await router.post(route("hospitals.appointments.delete", id));
      fetchAppointments();
    } catch (error) {
...
const handleAppointmentEdit = (id) => {
  router.visit(route("hospitals.appointments.edit", id));
};

watch([chamber_id, slot_id, startAt, endAt, particularDate], () => {
  fetchAppointments();
});

watch(chamber_id, getSlot);
