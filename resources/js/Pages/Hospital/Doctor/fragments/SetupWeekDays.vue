...
import { useConfig } from "@/Pages/Hospital/Doctor/useConfig";
import { Input } from "@/plugins/form";
import NextPrev from "@/Pages/Hospital/Doctor/NextPrev.vue";
import { onMounted, ref } from "vue";
import { Icon, Modal, Button } from "@/plugins/ui";
import { get } from "lodash";
import SlotDatePicker from '@/Pages/Hospital/Doctor/components/SlotDatePicker.vue'
...
const handleSave = (move) => {
    ...
    form.post(route("hospitals.doctors.save_weekdays", pageProps.doctor.id), {
        onSuccess(e) {
            if (!Object.values(e.props.errors).length) {
                location.href = route('hospitals.doctors.calender', props.doctor.id)
            }
        }
    })
};
