<template>
    <CompounderLayout title="Appointments" aside="compounder">
        <!-- Selected Slot and Chamber Information -->
        <div 
            v-if="activeSlot"
        ...
        <div class="mb-5" v-if="activeTab && !activeSlot">
            ...
            >
                ...
                <div class="flex flex-wrap justify-center gap-5">
                    <template v-for="slot in doctor_chamber.slots || []" :key="slot.id">
                        <div
        ...
        <div class="flex justify-center mt-5">
            <Button.Info :href="route('doctor.dashboard')">
                <Icon name="PhHouse" />
                Go Dashboard
            </Button.Info>
        </div>

        <!-- Modal for Editing/Adding Appointments -->
...
const handleSave = () => {
    ...
    form.post(route("doctor.appointments.saveAppointments", form.id), {
        onSuccess() {
            let index = appointments.value.findIndex(item => item.id == form.id)
            if(index > -1) {
                let item = appointments.value[index]
                item.id = form.id;
